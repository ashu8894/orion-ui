...
const Chatbot: React.FC = () => {
    ...
    const saveMessageToLocal = (threadId: string, message: Message) => {
        const existingThread = JSON.parse(localStorage.getItem(threadId) || '[]');
        const updatedThread = [...existingThread, message];
        localStorage.setItem(threadId, JSON.stringify(updatedThread));
    };
    ...
    const handleSend = async () => {
        ...
        if (threadId) saveMessageToLocal(threadId, userMessage);
        ...
        try {
            ...
            if (threadId) saveMessageToLocal(threadId, botMessage);
        } catch (error) {
            ...
            if (threadId) saveMessageToLocal(threadId, errorMessage);
        } finally {
            setIsTyping(false);
        }
    };
    ...
    useEffect(() => {
        const threadId = localStorage.getItem('currentThreadId');
        if (threadId) {
            const threadMessages = JSON.parse(localStorage.getItem(threadId) || '[]');
    ...
    return (
        ...
        >
            <div className='h-[95vh] w-[500px] rounded-[20px] overflow-hidden flex flex-col justify-between items-center shadow-lg'>
...
// thread_a4Fxi4Sktkw1Yawam4EIGBOY